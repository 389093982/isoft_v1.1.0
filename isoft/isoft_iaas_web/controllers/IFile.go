...
func (this *IFileController) GetImgOrVedioMedia() {
	...
	this.Ctx.ResponseWriter.Header().Set("Content-Type", ctype)
	this.Ctx.ResponseWriter.Header().Set("accept-ranges", "bytes")
	_version, err := strconv.Atoi(version)
	if err != nil{
		_version = 1
	}
	metadata, _ := models.GetMetadata(name, _version)
	this.Ctx.ResponseWriter.Header().Set("admin", "admin")
	this.Ctx.ResponseWriter.Header().Set("Content-Length", strconv.FormatInt(metadata.Size, 10))
	this.Ctx.ResponseWriter.Header().Set("Content-Range", fmt.Sprintf("bytes %d-%d/%d", 0, metadata.Size-1, metadata.Size))
	this.Ctx.ResponseWriter.Header().Set("Content-Disposition", fmt.Sprintf("attachment; filename=\"%s\"", name))
	this.Ctx.ResponseWriter.WriteHeader(http.StatusPartialContent)
	io.Copy(this.Ctx.ResponseWriter, res.Body)
	//http.ServeFile(this.Ctx.ResponseWriter, this.Ctx.Request, `D:/demo123.mp4`)
}
