...
import (
	...
	"isoft/isoft_deploy_web/deploy_core/task"
...
func main() {
	// 后面加个点可以解决 beego 在模板中数据无法嵌入数据的问题
	// {{template "common/header.html" .}}
	var AppNameFilter = func(ctx *context.Context) {
		ctx.Input.Data()["ServerName"] = beego.AppConfig.String("servername")
	}
	beego.InsertFilter("/*", beego.BeforeExec, AppNameFilter)
