...
func StartAllPackTask(packApps *PackApps, filterAppName string) (err error) {
	os.MkdirAll("D:/build/autopack/", os.ModePerm)
...
func StartPackCommonTask(appName, appPath string, packApp PackApp) (err error) {
	...
	// 移动文件
	err = os.Rename(fmt.Sprintf("./%s", appName), fmt.Sprintf("D:/build/autopack/%s", appName))
	return
}
...
func StartPackBeegoTask(appName, appPath string) (err error) {
	...
	// 移动文件
	err = os.Rename(fmt.Sprintf("./%s.tar.gz", appName), fmt.Sprintf("D:/build/autopack/%s.tar.gz", appName))
	return
}
